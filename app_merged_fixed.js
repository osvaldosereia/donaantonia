...
const MERGE_GROUPS = [
  {
    label: "Julgados",
    test: (o) => o.url.includes("data/julgados/")
  },
];
...
async function firstMatchInGroup(group, predicate) {
  for (const url of group.urls) {
    ...
    forEachBlockIncremental(txt, (block, idx) => {
      if (found) return;
      const it = parseBlock(block, idx, url, group.label);
...
// AGRUPA por conjuntos (ex.: une todos os /data/julgados/ em “Julgados”)
const mergedSources = buildMergedSources(allOptions);
”. Confira o rótulo do catálogo.`);
      }
    }
...
for (const src of mergedSources) {
  try {
    ...
    let first;
    if (src.type === "group") {
      first = await firstMatchInGroup(src, predicate);
      if (first) {
        lazyGroups.push({
          label: src.label,
          url: src.urls[0],
      ...
      first = await firstMatchInFile(src.url, src.label, predicate);
      if (first) {
    ...
    toast(`⚠️ Não carreguei: ${src.label || src.url}`);
    console.warn("Falha ao buscar:", e);
  }
}

// ... resto do doSearch continua igual (renderização final, foco, etc.)
      els.q?.select();
    }
